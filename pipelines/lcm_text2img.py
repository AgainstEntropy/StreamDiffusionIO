...
from diffusers.pipelines.latent_consistency_models.\
    pipeline_latent_consistency_text2img import retrieve_timesteps
...
class LatentConsistencyModelStreamIO:
    ...
    def _prepare(
        ...
        self.latent_shape = (self.unet.config.in_channels, ) + (self.resolution // self.vae_scale_factor, ) * 2
        ...
        prompt_embeds_shape = (self.num_inference_steps, self.tokenizer.model_max_length, self.text_encoder.config.hidden_size)
        self.prompt_embeds_batch = torch.zeros(
            prompt_embeds_shape, dtype=self.dtype, device=self.device
        )

        c_skip_out_list = torch.tensor([
            self.scheduler.get_scalings_for_boundary_condition_discrete(
                t
    ...
    def _encoder_new_prompt(self, new_prompt: str):
        ...
        new_prompt_embeds = self.text_encoder(text_input_ids.to(self.text_encoder.device)).last_hidden_state
    ...
    def scheduler_step_batch(
        ...
        self.prev_latent_buffer = self.alpha_prod_t_sqrt_list[1:] * prev_sample_batch + self.beta_prod_t_sqrt_list[1:] * self.init_noise[1:]
    ...
    def step_batch(self) -> torch.Tensor:
        ...
        image = self.vae.decode(denoised_x0 / self.vae.config.scaling_factor, return_dict=False)[0]
        do_denormalize = [True] * image.shape[0]
        image = self.image_processor.postprocess(image, output_type=self.output_type, do_denormalize=do_denormalize)[0]
